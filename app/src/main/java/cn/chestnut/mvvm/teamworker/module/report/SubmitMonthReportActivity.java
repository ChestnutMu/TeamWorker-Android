...
import cn.chestnut.mvvm.teamworker.databinding.ActivitySubmitMonthReportBinding;
import cn.chestnut.mvvm.teamworker.databinding.ActivitySubmitWeekReportBinding;
...
public class SubmitMonthReportActivity extends BaseActivity {

    private ActivitySubmitMonthReportBinding binding;
    ...
    protected void addContainerView(ViewGroup viewGroup, LayoutInflater inflater) {
        binding = DataBindingUtil.inflate(inflater, R.layout.activity_submit_month_report, viewGroup, true);
        initData();
        addListener();
    }
    ...
    protected void addListener() {
        binding.ivPicture.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                processPhotoUtils = new ProcessPhotoUtils(SubmitMonthReportActivity.this);
        ...
        binding.btnSubmit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (StringUtil.isStringNotNull(binding.etWorkContent.getText().toString())
                        && StringUtil.isStringNotNull(binding.etSummaryWork.getText().toString())
                        && StringUtil.isStringNotNull(binding.etPlanWork.getText().toString())) {
                    submitReport();
                } else {
                    ...
                    public void complete(String key, ResponseInfo info, JSONObject res) {
                        ...
                        if (info.isOK()) {
                            ...
                            try {
                                ...
                                showToast("图片上传成功");
                                GlideLoader.displayImage(SubmitMonthReportActivity.this, filePath, binding.ivPicture);
    ...
    private void submitReport() {
        ...
        params.put("workContent", binding.etWorkContent.getText().toString());
        params.put("workPlan", binding.etPlanWork.getText().toString());
        params.put("workSummary", binding.etSummaryWork.getText().toString());
        params.put("needCoordinatedWork", binding.etNeedCoordinatedWork.getText().toString());
        params.put("remarks", binding.etRemarks.getText().toString());
        if (StringUtil.isStringNotNull(pictureKey)) {
            params.put("photo", pictureKey);
        }
        showProgressDialog(this);
        RequestManager.getInstance(this).executeRequest(HttpUrls.SUBMIT_MONTH_REPORT, params, new AppCallBack<ApiResponse<Object>>() {
            @Override
